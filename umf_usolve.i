...
)
{
    ...
    long k, deg, j, *ip, col, *Upos, *Uilen, pos,
...
# 62 "umf_usolve.c"
 ...
 { (void) SuiteSparse_config.divcomplex_func ((xk).component [0], (xk).component [1], (D [k]).component [0], (D [k]).component [1], &((X [k]).component [0]), &((X [k]).component [1])) ; } ;
    }
...
# 140 "umf_usolve.c"
 ...
 if (deg > 0)
 {
  ...
  ;
  { ; (xk).component [0] -= (X [Ui [j]]).component [0] * (Uval [j]).component [0] - (X [Ui [j]]).component [1] * (Uval [j]).component [1] ; (xk).component [1] -= (X [Ui [j]]).component [1] * (Uval [j]).component [0] + (X [Ui [j]]).component [0] * (Uval [j]).component [1] ; } ;
     }
 }
