# 1 "UMFPACK/Source/umf_analyze.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_analyze.c"
# 28 "UMFPACK/Source/umf_analyze.c"
...
# 29 "UMFPACK/Source/umf_analyze.c" 2
...
# 30 "UMFPACK/Source/umf_analyze.c" 2
...
# 31 "UMFPACK/Source/umf_analyze.c" 2
...
# 32 "UMFPACK/Source/umf_analyze.c" 2
...
(
    ...
    long Ap [ ],
# 57 "UMFPACK/Source/umf_analyze.c"
    long Up [ ],
...
)
{
    ...
    long j, j3, col, k, row, parent, j2, pdest, p, p2, thickness, npivots, nfr,
...
#pragma ivdep
 ...
 {
# 163 "UMFPACK/Source/umf_analyze.c"
     pdest = 0 ;
     ncompactions++ ;
     for (row = 0 ; row < j ; row++)
     {
...
# 210 "UMFPACK/Source/umf_analyze.c"
 }
...
# 282 "UMFPACK/Source/umf_analyze.c"
 ;
 W [j] = Wflag ;
...
# 370 "UMFPACK/Source/umf_analyze.c"
 krow = knext ;
 pfirst = Ap [knext] ;
# 380 "UMFPACK/Source/umf_analyze.c"
 for (k = Link [j] ; k != (-1) ; k = Link [k])
 {
...
# 441 "UMFPACK/Source/umf_analyze.c"
 for (j2 = j+1 ; j2 < jnext ; j2++)
 {
     ;
     if (W [j2] != Wflag || Link [j2] != (-1))
     {
  break ;
     }
 }
...
# 518 "UMFPACK/Source/umf_analyze.c"
 npivcol = npivots ;
 fallrows = thickness ;
 fallcols = npivots + pdest - Up [j] ;


 fpiv = (((npivcol) < (fallrows)) ? (npivcol) : (fallrows)) ;
...
#pragma novector
 ...
 umf_l_fsize (n_col, Front_size, Front_nrows, Front_ncols,
     Front_parent, Front_npivcol) ;

 amd_l_postorder (n_col, Front_parent, Front_npivcol, Front_size,
     Front_order, Front_child, Front_sibling, Front_stack) ;
# 622 "UMFPACK/Source/umf_analyze.c"
 Winv = Ai ;
 for (k = 0 ; k < nfr ; k++)
 {
     Winv [k] = (-1) ;
...
# 673 "UMFPACK/Source/umf_analyze.c"
    umf_l_apply_order (Front_npivcol, Front_order, Ai, n_col, nfr) ;
    umf_l_apply_order (Front_nrows, Front_order, Ai, n_col, nfr) ;
    umf_l_apply_order (Front_ncols, Front_order, Ai, n_col, nfr) ;
    umf_l_apply_order (Front_parent, Front_order, Ai, n_col, nfr) ;
...
# 702 "UMFPACK/Source/umf_analyze.c"
