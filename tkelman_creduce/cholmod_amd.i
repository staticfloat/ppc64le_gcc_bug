# 1 "CHOLMOD/Cholesky/cholmod_amd.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "CHOLMOD/Cholesky/cholmod_amd.c"
# 35 "CHOLMOD/Cholesky/cholmod_amd.c"
...
# 36 "CHOLMOD/Cholesky/cholmod_amd.c" 2
...
# 37 "CHOLMOD/Cholesky/cholmod_amd.c" 2
...
# 38 "CHOLMOD/Cholesky/cholmod_amd.c" 2
# 47 "CHOLMOD/Cholesky/cholmod_amd.c"
int cholmod_l_amd
(
...
{
    double Info [20], Control2 [5], *Control ;
    long *Cp, *Len, *Nv, *Head, *Elen, *Degree, *Wi, *Iwork, *Next ;
    cholmod_sparse *C ;
    long j, n, cnz ;
    size_t s ;
    int ok = 1 ;
    ...
    { if (Common == 
# 70 "CHOLMOD/Cholesky/cholmod_amd.c" 3 4
   ((void *)0)
# 70 "CHOLMOD/Cholesky/cholmod_amd.c"
   ) { return (0) ; } if (Common->itype != 2 || Common->dtype != 0) { Common->status = (-4) ; return (0) ; } } ;
    { if ((A) == 
# 71 "CHOLMOD/Cholesky/cholmod_amd.c" 3 4
   ((void *)0)
# 71 "CHOLMOD/Cholesky/cholmod_amd.c"
   ) { if (Common->status != (-2)) { cholmod_l_error ((-4), "CHOLMOD/Cholesky/cholmod_amd.c", 71, "argument missing", Common) ; } return (0) ; } } ;
    n = A->nrow ;

    { if ((Perm) == 
# 74 "CHOLMOD/Cholesky/cholmod_amd.c" 3 4
   ((void *)0)
# 74 "CHOLMOD/Cholesky/cholmod_amd.c"
   ) { if (Common->status != (-2)) { cholmod_l_error ((-4), "CHOLMOD/Cholesky/cholmod_amd.c", 74, "argument missing", Common) ; } return (0) ; } } ;
    { if ((A)->xtype < (0) || (A)->xtype > (3) || ((A)->xtype != 0 && ((A)->x) == 
# 75 "CHOLMOD/Cholesky/cholmod_amd.c" 3 4
   ((void *)0)
# 75 "CHOLMOD/Cholesky/cholmod_amd.c"
   ) || ((A)->xtype == 3 && ((A)->z) == 
# 75 "CHOLMOD/Cholesky/cholmod_amd.c" 3 4
   ((void *)0)
# 75 "CHOLMOD/Cholesky/cholmod_amd.c"
   )) { if (Common->status != (-2)) { cholmod_l_error ((-4), "CHOLMOD/Cholesky/cholmod_amd.c", 75, "invalid xtype", Common) ; } return (0) ; } } ;
    Common->status = 0 ;
    if (n == 0)
    {
...
# 96 "CHOLMOD/Cholesky/cholmod_amd.c"
    s = cholmod_l_mult_size_t (n, 6, &ok) ;
    if (!ok)
    {
 cholmod_l_error ((-3), "CHOLMOD/Cholesky/cholmod_amd.c", 99, "problem too large", Common) ;
 return (0) ;
    }
    s = (((s) > (A->ncol)) ? (s) : (A->ncol)) ;
 ...
 return (0) ;
    ...
    Degree = Iwork ;
    Wi = Iwork + n ;
    Len = Iwork + 2*((size_t) n) ;
    Nv = Iwork + 3*((size_t) n) ;
    Next = Iwork + 4*((size_t) n) ;
    Elen = Iwork + 5*((size_t) n) ;
 ...
 C = cholmod_l_aat (A, fset, fsize, -2, Common) ;
    }
    else
    {
 ...
 Control = 
# 162 "CHOLMOD/Cholesky/cholmod_amd.c" 3 4
          ((void *)0) 
# 162 "CHOLMOD/Cholesky/cholmod_amd.c"
               ;
    }
    else
    {
 Control = Control2 ;
 Control [0] = Common->method [Common->current].prune_dense ;
 Control [1] = Common->method [Common->current].aggressive ;
    }


    amd_l2 (n, C->p, C->i, Len, C->nzmax, cnz, Nv, Next, Perm, Head, Elen,
     Degree, Wi, Control, Info) ;
# 182 "CHOLMOD/Cholesky/cholmod_amd.c"
    Common->fl = Info [10] + 2 * Info [11] + n ;
