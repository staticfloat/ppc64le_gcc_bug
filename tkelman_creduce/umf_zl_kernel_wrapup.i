# 1 "UMFPACK/Source/umf_kernel_wrapup.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_kernel_wrapup.c"
# 12 "UMFPACK/Source/umf_kernel_wrapup.c"
...
# 13 "UMFPACK/Source/umf_kernel_wrapup.c" 2
...
# 14 "UMFPACK/Source/umf_kernel_wrapup.c" 2
 ...
 *Wc, *Wp, *Frpos, *Fcpos, *Row_degree, *Col_degree, *Rperm_init,
 n_row, n_col, n_inner, zero_pivot, nan_pivot, n1 ;
    ...
    {
 pivot_value = D [k] ;
 { (d) = SuiteSparse_config.hypot_func ((pivot_value).component [0], (pivot_value).component [1]) ; } ;
 zero_pivot = ((d) == 0.) ;
 nan_pivot = ((d) != (d)) ;
 ...
 else
 {
  ...
  else if (!((Numeric->min_udiag) != (Numeric->min_udiag)))
  {

      Numeric->min_udiag = (((Numeric->min_udiag) < (d)) ? (Numeric->min_udiag) : (d)) ;
  }
     }
  ...
  Numeric->max_udiag = d ;
     ...
     else if (!((Numeric->max_udiag) != (Numeric->max_udiag)))
     {

  Numeric->max_udiag = (((Numeric->max_udiag) > (d)) ? (Numeric->max_udiag) : (d)) ;
     }
 }
    }
 ...
 Work->Upattern = (long *) 
# 191 "UMFPACK/Source/umf_kernel_wrapup.c" 3 4
                         ((void *)0) 
# 191 "UMFPACK/Source/umf_kernel_wrapup.c"
                              ;
    }

    ;
    ;
    if (Numeric->nnzpiv < n_inner && !((Numeric->min_udiag) != (Numeric->min_udiag)))
    {
...
# 278 "UMFPACK/Source/umf_kernel_wrapup.c"
    for (k = 0 ; k < npiv ; k++)
    {
 pivrow = Rperm [k] ;
 Wr [k] = Uilen [pivrow] ;
