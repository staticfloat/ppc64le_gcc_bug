# 1 "UMFPACK/Source/umf_garbage_collection.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_garbage_collection.c"
# 47 "UMFPACK/Source/umf_garbage_collection.c"
...
# 48 "UMFPACK/Source/umf_garbage_collection.c" 2
# 1 "UMFPACK/Source/umf_garbage_collection.h" 1





 void umfzl_garbage_collection
(
    NumericType *Numeric,
    WorkType *Work,
    long drnew,
...
# 49 "UMFPACK/Source/umf_garbage_collection.c" 2
...
# 79 "UMFPACK/Source/umf_garbage_collection.c"
    ...
    n_row = Work->n_row ;
    n_col = Work->n_col ;
# 96 "UMFPACK/Source/umf_garbage_collection.c"
    Numeric->ngarbage++ ;
# 105 "UMFPACK/Source/umf_garbage_collection.c"
    for (row = 0 ; row < n_row ; row++)
    {
 if ((Row_degree [row] >= 0) && Row_tuples [row])
 {
     ;
    ...
    {
 if ((Col_degree [col] >= 0) && Col_tuples [col])
 {
     ;
    ...
    nel = Work->nel ;
    ;
# 155 "UMFPACK/Source/umf_garbage_collection.c"
    e2 = 0 ;
 ...
 if (E [e])
 {
  ...
  Work->prior_element = e2 ;
    ...
    Work->nel = e2 ;
    nel = Work->nel ;
# 198 "UMFPACK/Source/umf_garbage_collection.c"
 ...
 if (e == 0)
 {
     ...
     DoubleComplex *F1, *F2, *Fsrc, *Fdst ;
     long c, r, k, dr, dc, gap, gap1, gap2, nb ;


     F1 = (DoubleComplex *) (psrc + 1) ;
  ...
  drnew++ ;
     ...
     drnew = (((dr) < (drnew)) ? (dr) : (drnew)) ;
     ;

     pnext = pdest ;
# 284 "UMFPACK/Source/umf_garbage_collection.c"
     Fsrc = Work->Flblock ;
     Fdst = Work->Flblock ;
     ;
     gap1 = dr - r ;
...
# 359 "UMFPACK/Source/umf_garbage_collection.c"
     }
# 374 "UMFPACK/Source/umf_garbage_collection.c"
     Work->fnr_curr = drnew ;
     Work->fnc_curr = dcnew ;
     Work->fcurr_size = (drnew + nb) * (dcnew + nb) ;
     size = (((((sizeof (DoubleComplex) * (Work->fcurr_size))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
  ...
  for (i = k-1 ; i >= 0 ; i--)
  {
     ...
     Work->Flublock = (DoubleComplex *) (Numeric->Memory + E [0]) ;
     ;
...
# 487 "UMFPACK/Source/umf_garbage_collection.c"
 }
 else if (e > 0)
 {
# 508 "UMFPACK/Source/umf_garbage_collection.c"
     p = psrc + 1 ;
     { { { ; ; ; epsrc = (Element *) p ; p += (((((sizeof (Element) * (1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; Cols = (long *) p ; } ; ncols = epsrc->ncols ; Rows = Cols + ncols ; } ; nrows = epsrc->nrows ; p += (((((sizeof (long) * (ncols + nrows))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; C = (DoubleComplex *) p ; } ;
     nrowsleft = epsrc->nrowsleft ;
     ncolsleft = epsrc->ncolsleft ;
     cdeg = epsrc->cdeg ;
     rdeg = epsrc->rdeg ;
# 529 "UMFPACK/Source/umf_garbage_collection.c"
     csize = nrowsleft * ncolsleft ;
     size2 = (((((sizeof (Element) * (1))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))
    + (((((sizeof (long) * (nrowsleft + ncolsleft))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))
    + (((((sizeof (DoubleComplex) * (csize))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
     ...
     p += (((((sizeof (long) * (nrowsleft + ncolsleft))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
     C2 = (DoubleComplex *) p ;
