# 1 "UMFPACK/Source/umf_start_front.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_start_front.c"
# 14 "UMFPACK/Source/umf_start_front.c"
...
# 15 "UMFPACK/Source/umf_start_front.c" 2
...
# 16 "UMFPACK/Source/umf_start_front.c" 2
...
# 17 "UMFPACK/Source/umf_start_front.c" 2
...
# 92 "UMFPACK/Source/umf_start_front.c"
 if (Symbolic->amd_dmax > 0)
 {
     cdeg = (((cdeg) < (Symbolic->amd_dmax)) ? (cdeg) : (Symbolic->amd_dmax)) ;
 }
 ...
 cdeg = (((cdeg) < (fnrows_max)) ? (cdeg) : (fnrows_max)) ;
 ...
 cdeg = 0 ;
    ...
    ;
    ;
# 146 "UMFPACK/Source/umf_start_front.c"
    overflow = ((!((maxbytes) * (1.0+1e-8) <= (double) 0x7fffffffffffffffL)) || ((maxbytes) != (maxbytes))) ;
    if (overflow)
    {

 maxfrsize = 0x7fffffffffffffffL / sizeof (DoubleComplex) ;
    }
    else
    {
 ...
 fsize = (((1) > (fsize)) ? (1) : (fsize)) ;
    }
    else
    {
 if (((!((Numeric->front_alloc_init * maxbytes) * (1.0+1e-8) <= (double) 0x7fffffffffffffffL)) || ((Numeric->front_alloc_init * maxbytes) != (Numeric->front_alloc_init * maxbytes))))
 {

     fsize = 0x7fffffffffffffffL / sizeof (DoubleComplex) ;
 }
 else
 {
 ...
 if (cdeg > 0)
 {
     ...
     if (((!((((double) cdeg * (double) cdeg) * sizeof (DoubleComplex)) * (1.0+1e-8) <= (double) 0x7fffffffffffffffL)) || ((((double) cdeg * (double) cdeg) * sizeof (DoubleComplex)) != (((double) cdeg * (double) cdeg) * sizeof (DoubleComplex)))))
     {

  fsize2 = 0x7fffffffffffffffL / sizeof (DoubleComplex) ;
     }
     else
     {
  fsize2 = (((cdeg * cdeg) > (fcurr_size)) ? (cdeg * cdeg) : (fcurr_size)) ;
     }
     fsize = (((fsize) < (fsize2)) ? (fsize) : (fsize2)) ;
 }
    }

    fsize = (((fsize) > (2*nb*nb)) ? (fsize) : (2*nb*nb)) ;
 ...
 {
     fnr2 = (long) sqrt ((double) fsize) ;

     fnr2 = (((fnr2) > (1)) ? (fnr2) : (1)) ;
     if (fnr2 % 2 == 0) fnr2++ ;
     fnr2 = (((fnr2) < (fnrows_max + nb)) ? (fnr2) : (fnrows_max + nb)) ;
     fnc2 = fsize / fnr2 ;
 }
 else
 {
     fnc2 = (long) sqrt ((double) fsize) ;
     fnc2 = (((fnc2) < (fncols_max + nb)) ? (fnc2) : (fncols_max + nb)) ;
     fnr2 = fsize / fnc2 ;
 ...
 ;
 Work->do_grow = (1) ;
 if (!umfzl_grow_front (Numeric, fnr2, fnc2, Work, -1))
 {
