# 1 "UMFPACK/Source/umf_local_search.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_local_search.c"
# 19 "UMFPACK/Source/umf_local_search.c"
...
# 20 "UMFPACK/Source/umf_local_search.c" 2
# 1 "UMFPACK/Source/umf_local_search.h" 1





 long umfzl_local_search
(
    NumericType *Numeric,
    WorkType *Work,
    SymbolicType *Symbolic
) ;
# 21 "UMFPACK/Source/umf_local_search.c" 2
# 1 "UMFPACK/Source/umf_row_search.h" 1





 long umfzl_row_search
(
    NumericType *Numeric,
    WorkType *Work,
    SymbolicType *Symbolic,
    ...
    long W_o [ ],
    long prior_pivrow [2],
    const DoubleComplex Wxy [ ],
    long pivcol,
    long freebie [2]
) ;
# 22 "UMFPACK/Source/umf_local_search.c" 2
# 1 "UMFPACK/Source/umf_mem_free_tail_block.h" 1
...
# 23 "UMFPACK/Source/umf_local_search.c" 2
# 36 "UMFPACK/Source/umf_local_search.c"
static void remove_candidate (long jj, WorkType *Work, SymbolicType *Symbolic)
{
# 55 "UMFPACK/Source/umf_local_search.c"
    if (Symbolic->fixQ)
    {
 ;
 ...
 else
 {
     ...
     Work->Candidates [Work->ncand - 1] = (-1) ;
     Work->nCandidates-- ;
 }
    }
    Work->ncand-- ;
# 103 "UMFPACK/Source/umf_local_search.c"
}
 ...
 *Flu ;
    long pos, nrows, *Cols, *Rows, e, f, status, max_cdeg, fnzeros, nb, j, col,
 ...
 *Diagonal_map, *Diagonal_imap, row2, col2 ;
    ...
    long blas_ok = (1) ;
# 151 "UMFPACK/Source/umf_local_search.c"
    ...
    nothing [0] = (-1) ;
    nothing [1] = (-1) ;
    relax1 = (Symbolic->prefer_diagonal) ? 0.0 : 0.25 ;
...
# 237 "UMFPACK/Source/umf_local_search.c"
    pivcol [0] = (-1) ;
    pivcol [1] = (-1) ;

    cdeg_in = 0x7fffffffffffffffL ;
    cdeg_out = 0x7fffffffffffffffL ;
    ...
    rdeg [0][0] = 0x7fffffffffffffffL ;
    rdeg [0][1] = 0x7fffffffffffffffL ;
    rdeg [1][0] = 0x7fffffffffffffffL ;
    rdeg [1][1] = 0x7fffffffffffffffL ;
    ...
    fncols_new [1][0] = (-1) ;
    fncols_new [1][1] = (-1) ;
# 318 "UMFPACK/Source/umf_local_search.c"
    ...
    deg = Symbolic->fixQ ? (-1) : Col_degree [col] ;
# 342 "UMFPACK/Source/umf_local_search.c"
    if (Fcpos [col] >= 0)
    {
 ...
 if (Fcpos [col] >= 0)
 {
     ...
     if (deg < cdeg_in || (deg == cdeg_in && col < pivcol [0]))
     {
 ...
 {
     if (deg < cdeg_out || (deg == cdeg_out && col < pivcol [1]))
     {
  ...
  jcand [1] = j ;
    ...
    cdeg_in = (-1) ;
    cdeg_out = (-1) ;
# 439 "UMFPACK/Source/umf_local_search.c"
    if (pivcol [0] != (-1))
    {
# 449 "UMFPACK/Source/umf_local_search.c"
 ;

 ;
 Fs = Fcblock + Fcpos [pivcol [0]] ;
 Fu = Fublock + (Fcpos [pivcol [0]] / fnr_curr) ;
...
# 473 "UMFPACK/Source/umf_local_search.c"
 if (fnpiv > 1)
 {



     { { int N = fnpiv, LDA = nb, INCX = 1 ; if ((sizeof (int) < sizeof (long)) && !((((int) N) == ((long) fnpiv)) && (((int) LDA) == ((long) nb)) && (((int) INCX) == ((long) 1)))) { blas_ok = (0) ; } if (!(sizeof (int) < sizeof (long)) || blas_ok) { ztrsv_ ("L", "N", "U", &N, (double *) Flublock, &LDA, (double *) Flu, &INCX) ; } } ; } ;

     if (!blas_ok)
     {

  DoubleComplex *Flub = Flublock ;
  for (j = 0 ; j < fnpiv ; j++)
  {
...
#pragma ivdep
   ...
   { ; (Flu [i]).component [0] -= (Flub [i]).component [0] * (Fuj).component [0] - (Flub [i]).component [1] * (Fuj).component [1] ; (Flu [i]).component [1] -= (Flub [i]).component [1] * (Fuj).component [0] + (Flub [i]).component [0] * (Fuj).component [1] ; } ;
      }
      Flub += nb ;
  }
     }
...
# 533 "UMFPACK/Source/umf_local_search.c"
 { double alpha [2] = {-1,0}, beta [2] = {1,0} ; { int M = fnrows, N = fnpiv, LDA = fnr_curr, INCX = 1, INCY = 1 ; if ((sizeof (int) < sizeof (long)) && !((((int) M) == ((long) fnrows)) && (((int) N) == ((long) fnpiv)) && (((int) LDA) == ((long) fnr_curr)) && (((int) INCX) == ((long) 1)) && (((int) INCY) == ((long) 1)))) { blas_ok = (0) ; } if (!(sizeof (int) < sizeof (long)) || blas_ok) { zgemv_ ("N", &M, &N, alpha, (double *) Flblock, &LDA, (double *) Flu, &INCX, beta, (double *) Wy, &INCY) ; } } ; } ;
# 553 "UMFPACK/Source/umf_local_search.c"
 cdeg_in = fnrows ;
# 608 "UMFPACK/Source/umf_local_search.c"
 ;
 ;
 ...
 if (tpi)
 {
  ...
  Rows = Cols + ep->ncols ;
  nrows = ep->nrows ;
  p += (((((sizeof (long) * (ep->ncols + nrows))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
  C = ((DoubleComplex *) p) + f * nrows ;
  ...
  for (i = 0 ; i < nrows ; i++)
  {
   ...
   else
   {
       ...
       ;
       { (Wy [pos]).component [0] += (C [i]).component [0] ; (Wy [pos]).component [1] += (C [i]).component [1] ; } ;
   }
      }
  }
  *tp2++ = *tp ;
     }
     Col_tlen [pivcol [0]] = tp2 - tp1 ;
 }
# 711 "UMFPACK/Source/umf_local_search.c"
 nr_in = cdeg_in - fnrows ;



 ;
# 726 "UMFPACK/Source/umf_local_search.c"
 status = umfzl_row_search (Numeric, Work, Symbolic,
     fnrows, cdeg_in, Frows, Frpos,
     pivrow [0], rdeg [0], Fcols, Wio, nothing, Wy,
     pivcol [0], freebie) ;
 ;
...
# 751 "UMFPACK/Source/umf_local_search.c"
 ;
 ...
 if (pivrow [0][0] != (-1))
 {
  ...
  (nc * (cdeg_in - 1)) ;
     ...
     fnrows_new [0][0] = (fnrows-1) + nr_in ;
     fncols_new [0][0] = (fncols-1) + nc ;
     ...
     do_update = fnpiv > 0 &&
  (((double) fnzeros) / ((double) new_LUsize)) > 0.1 ;
 ...
 if (pivrow [0][1] != (-1))
 {
  ...
  ;
  if (Fcpos [col] < 0) nc++ ;
# 863 "UMFPACK/Source/umf_local_search.c"
     }
     ;
  ...
  ((nr_in-1) * (rdeg [0][1]-1)) ;
     ...
     extra_cols = ((fncols-1) + nc ) - (rdeg [0][1] - 1) ;
     ;
     ;
     ...
     fnrows_new [0][1] = fnrows + (nr_in-1) ;
     fncols_new [0][1] = (fncols-1) + nc ;
     relaxed_front = fnrows_new [0][1] * fncols_new [0][1] ;
  ...
  do_update = fnpiv > 0 &&
      (((double) fnzeros) / ((double) new_LUsize)) > 0.125 ;
  ...
  fnrows_new [0][1] = cdeg_in - 1 ;
  fncols_new [0][1] = rdeg [0][1] - 1 ;
     ...
     if (bestcost == (-1) || thiscost < bestcost)
     {
  ...
  new_fnzeros = fnzeros ;
    ...
    {
 search_pivcol_out = search_pivcol_out && (pivrow [0][0] == (-1)) ;
    }

    if (search_pivcol_out)
    {
# 991 "UMFPACK/Source/umf_local_search.c"
 cdeg_out = 0 ;
# 1006 "UMFPACK/Source/umf_local_search.c"
 ;
 ;
 ...
 if (tpi)
 {
  ...
  for (i = 0 ; i < nrows ; i++)
  {
   ...
   else
   {
       ...
       { (Wx [pos]).component [0] += (C [i]).component [0] ; (Wx [pos]).component [1] += (C [i]).component [1] ; } ;
   }
      }
  }
  *tp2++ = *tp ;
     }
     Col_tlen [pivcol [1]] = tp2 - tp1 ;
 }
# 1086 "UMFPACK/Source/umf_local_search.c"
 status = umfzl_row_search (Numeric, Work, Symbolic,
     0, cdeg_out, Wm, Wp,
     pivrow [1], rdeg [1], Woi, Woo, pivrow [0], Wx,
     pivcol [1], freebie) ;
 ...
 if (status == (1))
 {
     ...
     ;
     remove_candidate (jcand [1], Work, Symbolic) ;
     Work->ndiscard++ ;


     ;
     Col_tlen [pivcol [1]] = 0 ;
     umfzl_mem_free_tail_block (Numeric, Col_tuples [pivcol [1]]) ;
     Col_tuples [pivcol [1]] = 0 ;
 ...
 if (pivrow [1][0] != (-1))
 {
  ...
  ;
  Wm [0] = pivrow [1][0] ;
  { (Wx [0]).component [0] = 0. ; (Wx [0]).component [1] = 0. ; } ;
  cdeg_out = 1 ;
  ;
     }
...
# 1205 "UMFPACK/Source/umf_local_search.c"
     }
     ;
  ...
  ((nc-1) * (cdeg_out-1)) ;
     ...
     fnrows_new [1][0] = (fnrows-1) + nr_out ;
     fncols_new [1][0] = fncols + (nc-1) ;
     relaxed_front = fnrows_new [1][0] * fncols_new [1][0] ;
  ...
  fnrows_new [1][0] = cdeg_out - 1 ;
  fncols_new [1][0] = rdeg [1][0] - 1 ;
     }
 ...
 if (pivrow [1][1] != (-1))
 {
  ...
  fnrows_new [1][1] = nr_out-1 ;
  fncols_new [1][1] = nc-1 ;
  relaxed_front = fnrows_new [1][1] * fncols_new [1][1] ;
     }
     else
     {
  ...
  if (nr_out == (-1))
  {
   ...
   ;
   if (Frpos [row] < 0 || Frpos [row] >= fnrows) nr_out++ ;
# 1368 "UMFPACK/Source/umf_local_search.c"
      }
      ;
  }
  ...
  for (i = 0 ; i < rdeg [1][1] ; i++)
  {
      col = Woo [i] ;
      ;
      if (Fcpos [col] < 0) nc++ ;
# 1390 "UMFPACK/Source/umf_local_search.c"
  }
  ;

  extra_cols = (fncols + (nc-1)) - (rdeg [1][1] - 1) ;
  ...
  fnrows_new [1][1] = fnrows + (nr_out-1) ;
  fncols_new [1][1] = fncols + (nc-1) ;
  relaxed_front = fnrows_new [1][1] * fncols_new [1][1] ;
  ...
  fnrows_new [1][1] = cdeg_out - 1 ;
  fncols_new [1][1] = rdeg [1][1] - 1 ;
     }
 ...
 case 0:
     ;
     ;
     Work->pivcol_in_front = (1) ;
     Work->pivrow_in_front = (1) ;
     ...
     Work->rrdeg = rdeg [0][0] ;
     jj = jcand [0] ;
     Work->fnrows_new = fnrows_new [0][0] ;
     Work->fncols_new = fncols_new [0][0] ;
     break ;
 ...
 case 1:
     ...
     ;
     Work->pivcol_in_front = (1) ;
     Work->pivrow_in_front = (0) ;
     Work->pivcol = pivcol [0] ;
     Work->pivrow = pivrow [0][1] ;
     Work->ccdeg = nr_in ;
     Work->Wrow = Wio ;
     Work->rrdeg = rdeg [0][1] ;
     jj = jcand [0] ;
     Work->fnrows_new = fnrows_new [0][1] ;
     Work->fncols_new = fncols_new [0][1] ;
     break ;

 case 2:
     ;
     ;
     Work->pivcol_in_front = (0) ;
     Work->pivrow_in_front = (1) ;
     Work->pivcol = pivcol [1] ;
     Work->pivrow = pivrow [1][0] ;
     Work->ccdeg = cdeg_out ;
     ...
     jj = jcand [1] ;
     Work->fnrows_new = fnrows_new [1][0] ;
     Work->fncols_new = fncols_new [1][0] ;
     break ;
 ...
 case 3:
     ...
     Work->rrdeg = rdeg [1][1] ;
     jj = jcand [1] ;
     Work->fnrows_new = fnrows_new [1][1] ;
     Work->fncols_new = fncols_new [1][1] ;
     break ;
    ...
    if (!Work->pivcol_in_front && pivcol [0] != (-1))
    {
...
# 1592 "UMFPACK/Source/umf_local_search.c"
    ;
    ;
...
# 1886 "UMFPACK/Source/umf_local_search.c"
    if (Work->do_extend)
    {
 ...
 if (row2 != Work->pivrow)
 {
     ...
     ;
     Diagonal_map [Work->pivcol] = (-(Work->pivrow)-2) ;
     Diagonal_imap [Work->pivrow] = Work->pivcol ;

     Diagonal_map [col2] = (-(row2)-2) ;
