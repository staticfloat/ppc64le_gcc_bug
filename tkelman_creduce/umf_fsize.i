# 1 "UMFPACK/Source/umf_fsize.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_fsize.c"
# 14 "UMFPACK/Source/umf_fsize.c"
...
# 15 "UMFPACK/Source/umf_fsize.c" 2
...
# 16 "UMFPACK/Source/umf_fsize.c" 2
...
)
{
 ...
 if (Npiv [j] > 0)
 {
     ...
     if (((!((((double) r) * ((double) c)) * (1.0+1e-8) <= (double) 0x7fffffffffffffffL)) || ((((double) r) * ((double) c)) != (((double) r) * ((double) c)))))
     {

  frsize = 0x7fffffffffffffffL ;
     }
    
                                ;
     Fsize [j] = (((Fsize [j]) > (frsize)) ? (Fsize [j]) : (frsize)) ;
     ;
     if (parent != (-1))
     {
  ...
  ;
  ;
  Fsize [parent] = (((Fsize [parent]) > (Fsize [j])) ? (Fsize [parent]) : (Fsize [j])) ;
