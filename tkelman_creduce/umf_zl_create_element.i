# 1 "UMFPACK/Source/umf_create_element.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_create_element.c"
# 16 "UMFPACK/Source/umf_create_element.c"
...
# 17 "UMFPACK/Source/umf_create_element.c" 2
...
# 18 "UMFPACK/Source/umf_create_element.c" 2
# 1 "UMFPACK/Source/umf_mem_alloc_element.h" 1
...
# 19 "UMFPACK/Source/umf_create_element.c" 2
...
# 20 "UMFPACK/Source/umf_create_element.c" 2
...
# 21 "UMFPACK/Source/umf_create_element.c" 2
...
# 22 "UMFPACK/Source/umf_create_element.c" 2





static void copy_column (long len, DoubleComplex *X, DoubleComplex *Y)
{
    long i ;
#pragma ivdep
    for (i = 0 ; i < len ; i++)
...
)
{
    ...
    long j, col, row, *Fcols, *Frows, fnrows, fncols, *Cols, len, needunits, t1,
 ...
 *Col_degree, *Col_tlen, nn, n_row, n_col, r2, c2, do_Fcpos ;
    DoubleComplex *C, *Fcol ;
    Element *ep ;
    Unit *p, *Memory ;
    Tuple *tp, *tp1, *tp2, tuple, *tpend ;
# 69 "UMFPACK/Source/umf_create_element.c"
    ;
    ;
    ...
    tp = (Tuple *) 
# 88 "UMFPACK/Source/umf_create_element.c" 3 4
                  ((void *)0) 
# 88 "UMFPACK/Source/umf_create_element.c"
                       ;
    tp1 = (Tuple *) 
# 89 "UMFPACK/Source/umf_create_element.c" 3 4
                   ((void *)0) 
# 89 "UMFPACK/Source/umf_create_element.c"
                        ;
    tp2 = (Tuple *) 
# 90 "UMFPACK/Source/umf_create_element.c" 3 4
                   ((void *)0) 
# 90 "UMFPACK/Source/umf_create_element.c"
                        ;
...
#pragma ivdep
 ...
 Row_degree [Frows [i]] += fncols ;
    ...
    E = Work->E ;
    max_mark = 0x7fffffffffffffffL - (2*(nn)+1) ;
...
# 214 "UMFPACK/Source/umf_create_element.c"
    needunits = 0 ;
    got_memory = (0) ;
    eloc = umfzl_mem_alloc_element (Numeric, fnrows, fncols, &Rows, &Cols, &C,
 &needunits, &ep) ;
 ...
 ;
 if (!umfzl_get_memory (Numeric, Work, needunits, r2, c2, do_Fcpos))
 {
...
#pragma ivdep
 ...
 for (tuple.f = 0 ; tuple.f < fncols ; tuple.f++)
 {
  ...
  p = Memory + t1 ;
  tp = (Tuple *) p ;
  size = (((p)-1)->header.size) ;
  len = Col_tlen [col] ;
  tp2 = tp + len ;
     }

     needunits = (((((sizeof (Tuple) * (len + 1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
  ...
  needunits = (((((sizeof (Tuple) * (len + 1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
     ...
     if (needunits > size)
     {
                               ...
                               ;
# 425 "UMFPACK/Source/umf_create_element.c"
  needunits = (((2*needunits) < ((long) (((((sizeof (Tuple) * (nn))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))))) ? (2*needunits) : ((long) (((((sizeof (Tuple) * (nn))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))))) ;
  t2 = umfzl_mem_alloc_tail_block (Numeric, needunits) ;
  if (!t2)
  {
      ...
      ;
      return (umfzl_get_memory (Numeric, Work, 0, r2, c2,do_Fcpos));
  }
  Col_tuples [col] = t2 ;
  tp2 = (Tuple *) (Memory + t2) ;
  if (t1)
   ...
   *tp2++ = *tp1++ ;
      ...
      umfzl_mem_free_tail_block (Numeric, t1) ;
  }
     }
 ...
 for (tuple.f = 0 ; tuple.f < fnrows ; tuple.f++)
 {
  ...
  needunits = (((((sizeof (Tuple) * (len + 1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
     ...
     if (needunits > size)
     {
                               ...
                               ;
# 528 "UMFPACK/Source/umf_create_element.c"
  needunits = (((2*needunits) < ((long) (((((sizeof (Tuple) * (nn))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))))) ? (2*needunits) : ((long) (((((sizeof (Tuple) * (nn))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))))) ;
  t2 = umfzl_mem_alloc_tail_block (Numeric, needunits) ;
  if (!t2)
  {
...
# 591 "UMFPACK/Source/umf_create_element.c"
    return ((1)) ;
}
