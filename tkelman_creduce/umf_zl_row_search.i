# 1 "UMFPACK/Source/umf_row_search.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_row_search.c"
# 20 "UMFPACK/Source/umf_row_search.c"
...
# 21 "UMFPACK/Source/umf_row_search.c" 2
...
# 22 "UMFPACK/Source/umf_row_search.c" 2
...
)
{
    ...
    long i, row, deg, col, *Frpos, fnrows, *E, j, ncols, *Cols, *Rows,
...
# 93 "UMFPACK/Source/umf_row_search.c"
    pivot [0] = 0. ;
    pivot [1] = 0. ;
 ...
 was_offdiag = (-1) ;
    ...
    for (i = 0 ; i < cdeg1 ; i++)
    {
 { (value) = (((((Wxy [i]).component [0]) < 0.)) ? -((Wxy [i]).component [0]) : ((Wxy [i]).component [0])) + (((((Wxy [i]).component [1]) < 0.)) ? -((Wxy [i]).component [1]) : ((Wxy [i]).component [1])) ; } ;
 if (((value) != (value)))
 {
     nans_in_col = (1) ;
     maxval = value ;
 ...
 if (prefer_diagonal)
 {
  ...
  { (a) = (((((Wxy [i]).component [0]) < 0.)) ? -((Wxy [i]).component [0]) : ((Wxy [i]).component [0])) + (((((Wxy [i]).component [1]) < 0.)) ? -((Wxy [i]).component [1]) : ((Wxy [i]).component [1])) ; } ;
...
# 348 "UMFPACK/Source/umf_row_search.c"
    if (cdeg1 > 0 && pivrow [0] == (-1) && pivrow [1] == (-1))
    {
 ...
 for (i = 1 ; i < cdeg1 ; i++)
 {
  ...
  ;
  if (deg < rdeg [0] || (deg == rdeg [0] && row == diag_row))
  {
  ...
  ;
  if (deg < rdeg [1] || (deg == rdeg [1] && row == diag_row))
  {
 ...
 if (fnrows > 0)
 {
# 439 "UMFPACK/Source/umf_row_search.c"
     pivrow [0] = Work->Frows [0] ;
 ...
 else
 {
     ...
     long *Front_leftmostdesc, *Front_1strow, *Front_new1strow, row1,
  row2, fleftmost, nfr, n_row, frontid ;
     ...
     frontid = Work->frontid ;
                    ...
                    ;
# 479 "UMFPACK/Source/umf_row_search.c"
...
# 558 "UMFPACK/Source/umf_row_search.c"
    if (pivrow [0] != (-1))
    {
...
# 599 "UMFPACK/Source/umf_row_search.c"
     rdeg [0] = fncols ;
  ...
  Row_tlen [pivrow [0]] = tp2 - tp1 ;
     }
# 692 "UMFPACK/Source/umf_row_search.c"
     Work->Wrpflag++ ;

 }
    }
# 720 "UMFPACK/Source/umf_row_search.c"
    if (pivrow [1] != (-1))
    {
 freebie [1] = (pivrow [1] == prior_pivrow [1]) && cdeg1 > 0 ;
 ;
 ...
 if (!freebie [1])
 {
  ...
  Row_tlen [pivrow [1]] = tp2 - tp1 ;
     }
# 816 "UMFPACK/Source/umf_row_search.c"
     Work->Wrpflag++ ;
    ...
    ;
# 835 "UMFPACK/Source/umf_row_search.c"
    return ((0)) ;
}
