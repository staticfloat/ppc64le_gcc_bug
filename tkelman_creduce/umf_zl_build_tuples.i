# 1 "UMFPACK/Source/umf_build_tuples.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "UMFPACK/Source/umf_build_tuples.c"
# 19 "UMFPACK/Source/umf_build_tuples.c"
...
# 20 "UMFPACK/Source/umf_build_tuples.c" 2
...
# 21 "UMFPACK/Source/umf_build_tuples.c" 2
...
# 22 "UMFPACK/Source/umf_build_tuples.c" 2
 ...
 *Col_tuples, *Col_degree, *Col_tlen, n1 ;
    ...
    n1 = Work->n1 ;
                     ...
                     ;
# 67 "UMFPACK/Source/umf_build_tuples.c"
    for (row = n1 ; row < n_row ; row++)
    {
 if ((Row_degree [row] >= 0))
 {
     Row_tuples [row] = umfzl_mem_alloc_tail_block (Numeric,
  (((((sizeof (Tuple) * ((((4) > ((Row_tlen [row]) + 1)) ? (4) : ((Row_tlen [row]) + 1))))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))) ;
     if (!Row_tuples [row])
     {
 ...
 if ((Col_degree [col] >= 0))
 {
     Col_tuples [col] = umfzl_mem_alloc_tail_block (Numeric,
  (((((sizeof (Tuple) * ((((4) > ((Col_tlen [col]) + 1)) ? (4) : ((Col_tlen [col]) + 1))))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))) ;
     if (!Col_tuples [col])
     {
...
# 110 "UMFPACK/Source/umf_build_tuples.c"
    for (e = 1 ; e <= nel ; e++)
    {
 ;
 ;
 p = Numeric->Memory + E [e] ;
 { { ; ; ; ep = (Element *) p ; p += (((((sizeof (Element) * (1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; Cols = (long *) p ; } ; ncols = ep->ncols ; Rows = Cols + ncols ; } ;
 nrows = ep->nrows ;
 ;
 ;
 ;
...
# 157 "UMFPACK/Source/umf_build_tuples.c"
    ;
    return ((1)) ;
}
