# 1 "CHOLMOD/Core/cholmod_change_factor.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "CHOLMOD/Core/cholmod_change_factor.c"
# 106 "CHOLMOD/Core/cholmod_change_factor.c"
...
# 107 "CHOLMOD/Core/cholmod_change_factor.c" 2
...
# 108 "CHOLMOD/Core/cholmod_change_factor.c" 2
...
# 1 "CHOLMOD/Core/t_cholmod_change_factor.c" 1
# 15 "CHOLMOD/Core/t_cholmod_change_factor.c"
# 1 "CHOLMOD/Include/cholmod_template.h" 1
# 16 "CHOLMOD/Core/t_cholmod_change_factor.c" 2





static void r_change_simplicial_numeric
(
    cholmod_factor *L,
    long to_ll,
    long to_packed,
...
)
{
    ...
    double *Lx, *Lz ;
    long *Lp, *Li, *Lnz ;
    long n, j, len, pnew, pold, k, p, pend ;
 ...
 for (j = 0 ; j < n ; j++)
 {
  ...
  if (((dj [0]) <= 0.))
  {
      ...
      if (L->minor == (size_t) n)
      {
   cholmod_l_error ((1), "CHOLMOD/Core/t_cholmod_change_factor.c", 91, "L not positive definite", Common) ;
   L->minor = j ;
      }
      for (k = 0 ; k < len ; k++)
      {
 ...
 cholmod_l_free (L->nzmax, sizeof (long), L->i, Common) ;


 cholmod_l_free (L->nzmax, sizeof (double), L->x, Common) ;
 ...
 if (make_ll)
 {
  ...
  if (((dj [0]) <= 0.))
  {
      ...
      if (L->minor == (size_t) n)
      {
   cholmod_l_error ((1), "CHOLMOD/Core/t_cholmod_change_factor.c", 246, "L not positive definite", Common) ;
   L->minor = j ;
      }
      for (k = 0 ; k < len ; k++)
      {
 ...
 for (j = 0 ; j < n ; j++)
 {
  ...
  if (L->minor == (size_t) n)
  {
      cholmod_l_error ((1), "CHOLMOD/Core/t_cholmod_change_factor.c", 382, "L not positive definite", Common) ;
      L->minor = j ;
  }
     }
     else
...
# 447 "CHOLMOD/Core/t_cholmod_change_factor.c"
static void r_ll_super_to_simplicial_numeric
(
    cholmod_factor *L,
    long to_packed,
    long to_ll,
...
)
{
    double ljj [1], lj2 [1] ;
    double *Lx ;
    long *Ls, *Lpi, *Lpx, *Super, *Lp, *Li, *Lnz ;
    long n, lnz, s, nsuper, p, psi, psx, psend, nsrow, nscol, ii, jj, j, k1, k2,
 q ;
...
# 117 "CHOLMOD/Core/cholmod_change_factor.c" 2
...
static void c_change_simplicial_numeric
(
    cholmod_factor *L,
    long to_ll,
    long to_packed,
...
)
{
 ...
 for (j = 0 ; j < n ; j++)
 {
  ...
  if (((dj [0]) <= 0.))
  {
   ...
   newLx [2*(pnew+k) ] = Lx [2*(pold+k) ] ; newLx [2*(pnew+k)+1] = Lx [2*(pold+k)+1] ;
      }
  }
  else
  {
   ...
   newLx [2*(pnew+k) ] = Lx [2*(pold+k) ] * ljj [2*(0)] ; newLx [2*(pnew+k)+1] = Lx [2*(pold+k)+1] * ljj [2*(0)];
      }
  }
 ...
 cholmod_l_free (L->nzmax, 2*sizeof (double), L->x, Common) ;
 ...
 if (make_ll)
 {
  ...
  if (((dj [0]) <= 0.))
  {
   ...
   Lx [2*(pnew+k) ] = Lx [2*(pold+k) ] ; Lx [2*(pnew+k)+1] = Lx [2*(pold+k)+1] ;
      }
  }
  else
  {
   ...
   Lx [2*(pnew+k) ] = Lx [2*(pold+k) ] * ljj [2*(0)] ; Lx [2*(pnew+k)+1] = Lx [2*(pold+k)+1] * ljj [2*(0)] ;
      }
  }
  Lp [j] = pnew ;
  pnew += len ;
 ...
 for (j = 0 ; j < n ; j++)
 {
  ...
  for (p++ ; p < pend ; p++)
  {
      ...
      Lx [2*(p) ] = Lx [2*(p) ] * ljj [2*(0)] ; Lx [2*(p)+1] = Lx [2*(p)+1] * ljj [2*(0)] ;
  }
     }
 }
...
# 447 "CHOLMOD/Core/t_cholmod_change_factor.c"
static void c_ll_super_to_simplicial_numeric
(
    cholmod_factor *L,
    long to_packed,
    long to_ll,
...
)
{
 ...
 for (jj = 0 ; jj < nscol ; jj++)
 {
  ...
  if (to_packed)
  {
   ...
   q = psx + ii + jj*nsrow ;
   Lx [2*(p) ] = Lx [2*(q) ] ; Lx [2*(p)+1] = Lx [2*(q)+1] ;
   ;
   ;
   ;
   p++ ;
  ...
  if (to_packed)
  {
   ...
   q = psx + ii + jj*nsrow ;
   Lx [2*(p) ] = Lx [2*(q) ] / ljj [2*(0)] ; Lx [2*(p)+1] = Lx [2*(q)+1] / ljj [2*(0)] ;
  ...
  else
  {
   ...
   q = psx + ii + jj*nsrow ;
   Lx [2*(q) ] = Lx [2*(q) ] / ljj [2*(0)] ; Lx [2*(q)+1] = Lx [2*(q)+1] / ljj [2*(0)] ;
      }
  }
     }
 }
...
# 119 "CHOLMOD/Core/cholmod_change_factor.c" 2
...
static void z_change_simplicial_numeric
(
    cholmod_factor *L,
    long to_ll,
    long to_packed,
...
)
{
 ...
 for (j = 0 ; j < n ; j++)
 {
  ...
  if (((dj [0]) <= 0.))
  {
   ...
   newLx [pnew+k] = Lx [pold+k] ; newLz [pnew+k] = Lz [pold+k] ;
      }
  }
  else
  {
   ...
   newLx [pnew+k] = Lx [pold+k] * ljj [0] ; newLz [pnew+k] = Lz [pold+k] * ljj [0];
      }
  }
 ...
 cholmod_l_free (L->nzmax, sizeof (double), L->x, Common) ;
 cholmod_l_free (L->nzmax, sizeof (double), L->z, Common) ;
 ...
 if (make_ll)
 {
  ...
  if (((dj [0]) <= 0.))
  {
   ...
   Lx [pnew+k] = Lx [pold+k] ; Lz [pnew+k] = Lz [pold+k] ;
      }
  }
  else
  {
   ...
   Lx [pnew+k] = Lx [pold+k] * ljj [0] ; Lz [pnew+k] = Lz [pold+k] * ljj [0] ;
      }
  }
  Lp [j] = pnew ;
  pnew += len ;
 ...
 for (j = 0 ; j < n ; j++)
 {
  ...
  for (p++ ; p < pend ; p++)
  {
      ...
      Lx [p] = Lx [p] * ljj [0] ; Lz [p] = Lz [p] * ljj [0] ;
  }
     }
 }
...
# 121 "CHOLMOD/Core/cholmod_change_factor.c" 2
# 169 "CHOLMOD/Core/cholmod_change_factor.c"
static void natural_list (cholmod_factor *L)
{
    long head, tail, n, j ;
    long *Lnext, *Lprev ;
...
# 200 "CHOLMOD/Core/cholmod_change_factor.c"
...
)
{
    ...
    Lp = cholmod_l_malloc (n1, sizeof (long), Common) ;
    Lnz = cholmod_l_malloc (n, sizeof (long), Common) ;
    Lprev = cholmod_l_malloc (n2, sizeof (long), Common) ;
    Lnext = cholmod_l_malloc (n2, sizeof (long), Common) ;

    if (Common->status < 0)
    {
 cholmod_l_free (n1, sizeof (long), Lp, Common) ;
 cholmod_l_free (n, sizeof (long), Lnz, Common) ;
 cholmod_l_free (n2, sizeof (long), Lprev, Common) ;
 cholmod_l_free (n2, sizeof (long), Lnext, Common) ;
 ;
 return (0) ;
    }
...
# 259 "CHOLMOD/Core/cholmod_change_factor.c"
...
{
    long nsuper, xsize, ssize ;
    long *Lsuper, *Lpi, *Lpx, *Ls ;
    size_t nsuper1 ;
    ...
    Lsuper = cholmod_l_malloc (nsuper1, sizeof (long), Common) ;
    Lpi = cholmod_l_malloc (nsuper1, sizeof (long), Common) ;
    Lpx = cholmod_l_malloc (nsuper1, sizeof (long), Common) ;


    Ls = cholmod_l_malloc (ssize, sizeof (long), Common) ;

    if (Common->status < 0)
    {
 cholmod_l_free (nsuper1, sizeof (long), Lsuper, Common) ;
 cholmod_l_free (nsuper1, sizeof (long), Lpi, Common) ;
 cholmod_l_free (nsuper1, sizeof (long), Lpx, Common) ;
 cholmod_l_free (ssize, sizeof (long), Ls, Common) ;
 return (0) ;
    }
...
# 326 "CHOLMOD/Core/cholmod_change_factor.c"
...
)
{
    ...
    L->p = cholmod_l_free (n1, sizeof (long), L->p, Common) ;
    L->i = cholmod_l_free (lnz, sizeof (long), L->i, Common) ;
    L->x = cholmod_l_free (xs, e*sizeof (double), L->x, Common) ;
    L->z = cholmod_l_free (lnz, sizeof (double), L->z, Common) ;
    L->nz = cholmod_l_free (n, sizeof (long), L->nz, Common) ;
    L->next = cholmod_l_free (n2, sizeof (long), L->next, Common) ;
    L->prev = cholmod_l_free (n2, sizeof (long), L->prev, Common) ;
    L->super = cholmod_l_free (s, sizeof (long), L->super, Common) ;
    L->pi = cholmod_l_free (s, sizeof (long), L->pi, Common) ;
    L->px = cholmod_l_free (s, sizeof (long), L->px, Common) ;
    L->s = cholmod_l_free (ss, sizeof (long), L->s, Common) ;
    L->nzmax = 0 ;
    L->is_super = 0 ;
    L->xtype = 0 ;
    L->dtype = 0 ;
...
# 376 "CHOLMOD/Core/cholmod_change_factor.c"
...
)
{
    ...
    ;
    ;
    L->x = cholmod_l_free (L->xsize,
     (L->xtype == 2 ? 2 : 1) * sizeof (double), L->x,
     Common) ;
    L->xtype = 0 ;
    L->dtype = 0 ;
    L->minor = L->n ;
...
# 409 "CHOLMOD/Core/cholmod_change_factor.c"
...
)
{
    ...
    double *Lx, *Lz ;
    long *Li, *Lp, *Lnz, *ColCount ;
    long n, grow, grow2, p, j, lnz, len, ok, e ;
 ...
 grow1 = Common->grow1 ;
 grow2 = Common->grow2 ;
 grow0 = ((grow0) != (grow0)) ? 1 : grow0 ;
 grow1 = ((grow1) != (grow1)) ? 1 : grow1 ;
 ...
 if (ok)
 {
  ...
  xlnz = (double) lnz ;
  xlnz *= grow0 ;
  xlnz = (((xlnz) < (((size_t) (-1)))) ? (xlnz) : (((size_t) (-1)))) ;
  xlnz = (((xlnz) < (((double) n * (double) n + (double) n) / 2)) ? (xlnz) : (((double) n * (double) n + (double) n) / 2)) ;
  lnz = (long) xlnz ;
     }
 }
    }
    ...
    if (!ok)
    {
 cholmod_l_error ((-3), "CHOLMOD/Core/cholmod_change_factor.c", 539, "problem too large", Common) ;
    }
    ...
    ;
    e = (to_xtype == 2 ? 2 : 1) ;
    if (!ok || !cholmod_l_realloc_multiple (lnz, 1, to_xtype, &(L->i), 
# 546 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
                                                                     ((void *)0)
# 546 "CHOLMOD/Core/cholmod_change_factor.c"
                                                                         ,
  &(L->x), &(L->z), &(L->nzmax), Common))
    {
 L->p = cholmod_l_free (n+1, sizeof (long), L->p, Common) ;
 L->nz = cholmod_l_free (n, sizeof (long), L->nz, Common) ;
 L->prev = cholmod_l_free (n+2, sizeof (long), L->prev, Common) ;
 L->next = cholmod_l_free (n+2, sizeof (long), L->next, Common) ;
 L->i = cholmod_l_free (lnz, sizeof (long), L->i, Common) ;
 L->x = cholmod_l_free (lnz, e*sizeof (double), L->x, Common) ;
 L->z = cholmod_l_free (lnz, sizeof (double), L->z, Common) ;
 ;
 return ;
    }
    ...
    Lx = L->x ;
    Lz = L->z ;
# 595 "CHOLMOD/Core/cholmod_change_factor.c"
    if (packed >= 0)
    {
...
# 668 "CHOLMOD/Core/cholmod_change_factor.c"
...
)
{
    ...
    double *Lx, *Lz ;
    long *Lp, *Li, *Lnz ;
    long make_monotonic, grow2, n, j, lnz, len, grow, ok, make_ll, make_ldl ;
    size_t nzmax0 ;
    ...
    ok = 1 ;
    newLi = 
# 707 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
           ((void *)0) 
# 707 "CHOLMOD/Core/cholmod_change_factor.c"
                ;
    newLx = 
# 708 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
           ((void *)0) 
# 708 "CHOLMOD/Core/cholmod_change_factor.c"
                ;
    newLz = 
# 709 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
           ((void *)0) 
# 709 "CHOLMOD/Core/cholmod_change_factor.c"
                ;
    lnz = 0 ;
 ...
 if (!ok)
 {
     cholmod_l_error ((-3), "CHOLMOD/Core/cholmod_change_factor.c", 752, "problem too large", Common) ;
     return ;
 }
 ...
 cholmod_l_realloc_multiple (lnz, 1, L->xtype, &newLi, 
# 769 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
                                                     ((void *)0)
# 769 "CHOLMOD/Core/cholmod_change_factor.c"
                                                         ,
...
# 818 "CHOLMOD/Core/cholmod_change_factor.c"
...
{
    long *Ls, *Lpi, *Lpx, *Super, *Li ;
    long n, lnz, s, nsuper, psi, psend, nsrow, nscol, k1, k2, erows ;
    ...
    if (to_packed)
    {
# 857 "CHOLMOD/Core/cholmod_change_factor.c"
 lnz = 0 ;
 for (s = 0 ; s < nsuper ; s++)
 {
     k1 = Super [s] ;
 ...
 lnz = L->xsize ;
    ...
    Li = cholmod_l_malloc (lnz, sizeof (long), Common) ;
    if (Common->status < 0)
    {
 return ;
    }
    ...
    if (!allocate_simplicial_numeric (L, Common))
    {
 cholmod_l_free (lnz, sizeof (long), Li, Common) ;
 return ;
    }
 ...
 case 2:
    ...
    L->super = cholmod_l_free (nsuper+1, sizeof (long), L->super, Common) ;
    L->pi = cholmod_l_free (nsuper+1, sizeof (long), L->pi, Common) ;
    L->px = cholmod_l_free (nsuper+1, sizeof (long), L->px, Common) ;
    L->s = cholmod_l_free (L->ssize, sizeof (long), L->s, Common) ;
...
# 949 "CHOLMOD/Core/cholmod_change_factor.c"
...
{
    double *Lx ;
    long wentry = (to_xtype == 1) ? 1 : 2 ;
    ;
    ;
    Lx = cholmod_l_malloc (L->xsize, wentry * sizeof (double), Common) ;
    ;
    if (Common->status < 0)
    {
 return (0) ;
...
# 1008 "CHOLMOD/Core/cholmod_change_factor.c"
...
)
{
    ...
    { if (Common == 
# 1028 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
   ((void *)0)
# 1028 "CHOLMOD/Core/cholmod_change_factor.c"
   ) { return (0) ; } if (Common->itype != 2 || Common->dtype != 0) { Common->status = (-4) ; return (0) ; } } ;
    { if ((L) == 
# 1029 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
   ((void *)0)
# 1029 "CHOLMOD/Core/cholmod_change_factor.c"
   ) { if (Common->status != (-2)) { cholmod_l_error ((-4), "CHOLMOD/Core/cholmod_change_factor.c", 1029, "argument missing", Common) ; } return (0) ; } } ;
    { if ((L)->xtype < (0) || (L)->xtype > (3) || ((L)->xtype != 0 && ((L)->x) == 
# 1030 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
   ((void *)0)
# 1030 "CHOLMOD/Core/cholmod_change_factor.c"
   ) || ((L)->xtype == 3 && ((L)->z) == 
# 1030 "CHOLMOD/Core/cholmod_change_factor.c" 3 4
   ((void *)0)
# 1030 "CHOLMOD/Core/cholmod_change_factor.c"
   )) { if (Common->status != (-2)) { cholmod_l_error ((-4), "CHOLMOD/Core/cholmod_change_factor.c", 1030, "invalid xtype", Common) ; } return (0) ; } } ;
    if (to_xtype < 0 || to_xtype > 3)
    {
 cholmod_l_error ((-4), "CHOLMOD/Core/cholmod_change_factor.c", 1033, "xtype invalid", Common) ;
 return (0) ;
    }
    Common->status = 0 ;
    ...
    to_ll = ((to_ll) ? 1 : 0) ;
    to_super = ((to_super) ? 1 : 0) ;

    ;
    ;

    if (to_super && to_xtype == 3)
    {
 cholmod_l_error ((-4), "CHOLMOD/Core/cholmod_change_factor.c", 1051, "supernodal zomplex L not supported", Common) ;
 return (0) ;
    }
...
# 1101 "CHOLMOD/Core/cholmod_change_factor.c"
  ,
 1102
# 1101 "CHOLMOD/Core/cholmod_change_factor.c"
 ...
 if (to_super)
 {
  ...
  if (!(L->is_super))
  {
      cholmod_l_error ((-4),
 "CHOLMOD/Core/cholmod_change_factor.c"
# 1155 "CHOLMOD/Core/cholmod_change_factor.c"
      ,
 1156
# 1155 "CHOLMOD/Core/cholmod_change_factor.c"
      , "cannot convert simplicial L to supernodal", Common)
                                                ;
  }
 ...
 else
 {
  ...
  simplicial_symbolic_to_simplicial_numeric (L, to_ll, to_packed,
   to_xtype, Common) ;
     ...
     {
# 1215 "CHOLMOD/Core/cholmod_change_factor.c"
