# 1 "umfpack_get_determinant.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "umfpack_get_determinant.c"
# 24 "umfpack_get_determinant.c"
# 1 "umf_internal.h" 1
# 25 "umf_internal.h"
...
# 25 "umfpack_get_determinant.c" 2
...
# 26 "umfpack_get_determinant.c" 2
# 1 "umf_malloc.h" 1
# 18 "umf_malloc.h"
 void *umf_l_malloc
(
...
# 27 "umfpack_get_determinant.c" 2
# 1 "umf_free.h" 1
...
# 28 "umfpack_get_determinant.c" 2
...
static long rescale_determinant
(
    DoubleComplex *d_mantissa,
    double *d_exponent
)
{
    double d_abs ;

    { (d_abs) = SuiteSparse_config.hypot_func ((*d_mantissa).component [0], (*d_mantissa).component [1]) ; } ;
 ...
 *d_exponent = 0 ;
 ...
 return ((0)) ;
    ...
    while (d_abs < 1.)
    {
 { (*d_mantissa).component [0] *= (10.0) ; (*d_mantissa).component [1] *= (10.0) ; } ;
 *d_exponent = *d_exponent - 1.0 ;
 { (d_abs) = SuiteSparse_config.hypot_func ((*d_mantissa).component [0], (*d_mantissa).component [1]) ; } ;
    ...
    while (d_abs >= 10.)
    {
 { (*d_mantissa).component [0] *= (0.1) ; (*d_mantissa).component [1] *= (0.1) ; } ;
 *d_exponent = *d_exponent + 1.0 ;
 { (d_abs) = SuiteSparse_config.hypot_func ((*d_mantissa).component [0], (*d_mantissa).component [1]) ; } ;
    }
...
)
{
    ...
    DoubleComplex d_mantissa, d_tmp ;
    double d_exponent, Info2 [90], one [2] = {1.0, 0.0}, d_sign ;
    DoubleComplex *D ;
    double *Info, *Rs ;
    NumericType *Numeric ;
    long i, n, itmp, npiv, *Wi, *Rperm, *Cperm, do_scale ;
    ...
    if (User_Info != (double *) 
# 109 "umfpack_get_determinant.c" 3 4
                               ((void *)0)
# 109 "umfpack_get_determinant.c"
                                   )
    {
 ...
 return ((-13)) ;
    ...
    if (Mx == (double *) 
# 140 "umfpack_get_determinant.c" 3 4
                        ((void *)0)
# 140 "umfpack_get_determinant.c"
                            )
    {
 Info [0] = (-5) ;
 return ((-5)) ;
    ...
    Wi = (long *) umf_l_malloc (n, sizeof (long)) ;
 ...
 return ((-1)) ;
    ...
    Rs = Numeric->Rs ;
    do_scale = (Rs != (double *) 
# 166 "umfpack_get_determinant.c" 3 4
                                ((void *)0)
# 166 "umfpack_get_determinant.c"
                                    ) ;
    ...
    d_mantissa = ((DoubleComplex *) one) [0] ;
    d_exponent = 0.0 ;
    D = Numeric->D ;
    ...
    for (i = 0 ; i < n ; i++)
    {
 { ; (d_tmp).component [0] = (d_mantissa).component [0] * (D [i]).component [0] - (d_mantissa).component [1] * (D [i]).component [1] ; (d_tmp).component [1] = (d_mantissa).component [1] * (D [i]).component [0] + (d_mantissa).component [0] * (D [i]).component [1] ; } ;
 d_mantissa = d_tmp ;
 ...
 for (i = 0 ; i < n ; i++)
 {
  ...
  { (d_mantissa).component [0] /= (Rs [i]) ; (d_mantissa).component [1] /= (Rs [i]) ; } ;
     }
     else
 ...
 while (Wi [i] != i)
 {
    ...
    if (Ex == (double *) 
# 274 "umfpack_get_determinant.c" 3 4
                        ((void *)0)
# 274 "umfpack_get_determinant.c"
                            )
    {

 { (d_mantissa).component [0] *= (pow (10.0, d_exponent)) ; (d_mantissa).component [1] *= (pow (10.0, d_exponent)) ; } ;
    }
    else
    {
 Ex [0] = d_exponent ;
    ...
    Mx [0] = d_sign * ((d_mantissa).component [0]) ;


    if (((Mz) != (double *) 
# 287 "umfpack_get_determinant.c" 3 4
       ((void *)0)
# 287 "umfpack_get_determinant.c"
       ))
    {
 Mz [0] = d_sign * ((d_mantissa).component [1]) ;
    }
    else
    {
 Mx [1] = d_sign * ((d_mantissa).component [1]) ;
    }



    if (d_exponent + 1.0 > log10 (((double)1.79769313486231570814527423731704e+308L)))
    {
 Info [0] = (3) ;
    }
    else if (d_exponent - 1.0 < log10 (((double)2.22507385850720138309023271733240e-308L)))
    {
 Info [0] = (2) ;
    }
