# 1 "umf_multicompile.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "umf_multicompile.c"
# 36 "umf_multicompile.c"
# 1 "umf_ltsolve.c" 1
# 14 "umf_ltsolve.c"
# 1 "umf_internal.h" 1
# 25 "umf_internal.h"
...
)
{
    ...
    long k, deg, *ip, j, row, *Lpos, *Lilen, kstart, kend, *Lip, llen,
 lp, pos, npiv, n1, *Li ;
...
# 58 "umf_ltsolve.c"
 ...
 for (k = kend ; k >= kstart ; k--)
 {
  ...
  lp = -lp ;
     ...
     ;
     llen = Lilen [k] ;
     xp = (DoubleComplex *) (Numeric->Memory + lp + (((((sizeof (long) * (llen))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))) ;
     xk = X [k] ;
     for (j = 0 ; j < deg ; j++)
     {
  ;
  ...
  { ; (xk).component [0] -= (X [Pattern [j]]).component [0] * (*xp).component [0] + (X [Pattern [j]]).component [1] * (*xp).component [1] ; (xk).component [1] -= (X [Pattern [j]]).component [1] * (*xp).component [0] - (X [Pattern [j]]).component [0] * (*xp).component [1] ; } ;
 ...
 if (deg > 0)
 {
  ...
  { ; (xk).component [0] -= (X [Li [j]]).component [0] * (Lval [j]).component [0] + (X [Li [j]]).component [1] * (Lval [j]).component [1] ; (xk).component [1] -= (X [Li [j]]).component [1] * (Lval [j]).component [0] - (X [Li [j]]).component [0] * (Lval [j]).component [1] ; } ;
...
# 224 "umf_ltsolve.c"
    return (8. * ((double) Numeric->lnz)) ;
}
# 37 "umf_multicompile.c" 2
# 1 "umf_utsolve.c" 1
# 15 "umf_utsolve.c"
# 1 "umf_utsolve.h" 1
...
)
{
    ...
    DoubleComplex xk ;
    DoubleComplex *xp, *D, *Uval ;
    long k, deg, j, *ip, col, *Upos, *Uilen, kstart, kend, up,
...
# 66 "umf_utsolve.c"
 ...
 { (void) SuiteSparse_config.divcomplex_func ((X [k]).component [0], (X [k]).component [1], (D [k]).component [0], (-(D [k]).component [1]), &((xk).component [0]), &((xk).component [1])) ; } ;
# 93 "umf_utsolve.c"
 X [k] = xk ;
 deg = Uilen [k] ;
 if (deg > 0 && ((((xk).component [0]) != 0.) || (((xk).component [1]) != 0.)))
 {
     up = Uip [k] ;
  ...
  { ; (X [Ui [j]]).component [0] -= (xk).component [0] * (Uval [j]).component [0] + (xk).component [1] * (Uval [j]).component [1] ; (X [Ui [j]]).component [1] -= (xk).component [1] * (Uval [j]).component [0] - (xk).component [0] * (Uval [j]).component [1] ; } ;
...
# 276 "umf_utsolve.c"
     X [k] = xk ;
     if (((((xk).component [0]) != 0.) || (((xk).component [1]) != 0.)))
     {
  if (k == kstart)
  {
      up = -up ;
      xp = (DoubleComplex *) (Numeric->Memory + up + (((((sizeof (long) * (ulen))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))) ;
  }
  else
  {
      xp = (DoubleComplex *) (Numeric->Memory + up) ;
  }
  for (j = 0 ; j < deg ; j++)
  {
      ;
      ...
      { ; (X [Pattern [j]]).component [0] -= (xk).component [0] * (*xp).component [0] + (xk).component [1] * (*xp).component [1] ; (X [Pattern [j]]).component [1] -= (xk).component [1] * (*xp).component [0] - (xk).component [0] * (*xp).component [1] ; } ;
...
# 330 "umf_utsolve.c"
    return (9. * ((double) n) + 8. * ((double) Numeric->unz));
}
# 38 "umf_multicompile.c" 2
...
)
{
    ...
    long i, j, k, p, cp, p1, p2, pdest, pj ;
...
)
{
    ...
    long split = ((Tz) != (double *) 
# 75 "umf_triplet.c" 3 4
               ((void *)0)
# 75 "umf_triplet.c"
               ) && ((Az) != (double *) 
# 75 "umf_triplet.c" 3 4
...
)
{
    ...
    long tpi, e, *E, *Fcpos, *Frpos, *Row_degree, *Row_tuples, *Row_tlen, rdeg0,
 ...
 if (ep->rdeg == rdeg0)
 {
     ...
     p += (((((sizeof (long) * (ncols + nrows))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
     S = ((DoubleComplex *) p) + f ;
...
#pragma ivdep
  for (j = 0 ; j < ncols ; j++)
  {
      ...
      { (Frow [Fcpos [col]]).component [0] += (*S).component [0] ; (Frow [Fcpos [col]]).component [1] += (*S).component [1] ; } ;
      S += nrows ;
  }
...
)
{
    ...
    long tpi, e, *E, *Fcpos, *Frpos, *Row_degree, *Col_tuples, *Col_tlen, cdeg0,
...
#pragma ivdep
  for (i = 0 ; i < nrows ; i++)
  {
      ...
      { (Fcol [Frpos [row]]).component [0] += (S [i]).component [0] ; (Fcol [Frpos [row]]).component [1] += (S [i]).component [1] ; } ;
  }
     }
     else
     {
...
)
{
    ...
    long e, i, row, col, i2, nrows, ncols, f, tpi, extcdeg, extrdeg, rdeg0,
...
# 420 "umf_assemble.c"
    ...
    Numeric->maxnrows = (((Numeric->maxnrows) > (fnrows)) ? (Numeric->maxnrows) : (fnrows)) ;
    Numeric->maxncols = (((Numeric->maxncols) > (fncols)) ? (Numeric->maxncols) : (fncols)) ;



    Numeric->maxfrsize = (((Numeric->maxfrsize) > (fnrows * fncols)) ? (Numeric->maxfrsize) : (fnrows * fncols)) ;
 ...
 { { { ; ; ; ep = (Element *) p ; p += (((((sizeof (Element) * (1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; Cols = (long *) p ; } ; ncols = ep->ncols ; Rows = Cols + ncols ; } ; nrows = ep->nrows ; p += (((((sizeof (long) * (ncols + nrows))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; S = (DoubleComplex *) p ; } ;
 nrowsleft = ep->nrowsleft ;
 ncolsleft = ep->ncolsleft ;
 next = ep->next ;
 ep->next = (-1) ;
...
#pragma ivdep
   for (i = 0 ; i < nrows ; i++)
   {
       ...
       { (Fcol [Wm [i]]).component [0] += (S [i]).component [0] ; (Fcol [Wm [i]]).component [1] += (S [i]).component [1] ; } ;
   }
   S += nrows ;
      }
...
#pragma ivdep
   for (i = 0 ; i < nrowsleft ; i++)
   {
       ...
       { (Fcol [Wm [i]]).component [0] += (S [Woo [i]]).component [0] ; (Fcol [Wm [i]]).component [1] += (S [Woo [i]]).component [1] ; } ;
   }
   S += nrows ;
      }
...
#pragma ivdep
   ...
   S += nrows ;
     ...
     umfzl_mem_free_tail_block (Numeric, E [e]) ;
     E [e] = 0 ;
...
)
{
    ...
    double Info2 [90], stats [2] ;
    double *Info ;
    NumericType *Numeric ;
    long n, i, irstep, status ;
# 90 "umfpack_solve.c"
    umfpack_tic (stats) ;
# 102 "umfpack_solve.c"
    irstep = ((Control != (double *) 
# 102 "umfpack_solve.c" 3 4
            ((void *)0)
# 102 "umfpack_solve.c"
            ) ? (((Control [7]) != (Control [7])) ? 2 : Control [7]) : 2) ;

    if (User_Info != (double *) 
# 104 "umfpack_solve.c" 3 4
                               ((void *)0)
# 104 "umfpack_solve.c"
                                   )
    {
 ...
 return ((-3)) ;
    ...
    Info [1] = Numeric->n_row ;
    Info [16] = Numeric->n_col ;
...
# 215 "umfpack_solve.c"
    status = umfzl_solve (sys, Ap, Ai, Ax, Xx, Bx,
