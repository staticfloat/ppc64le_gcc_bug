...
)
{
    ...
    long k, deg, *ip, j, row, *Lpos, *Lilen, *Lip, llen, lp, newLchain,
 pos, npiv, n1, *Li ;
...
# 52 "umf_lsolve.c"
 ...
 if (deg > 0 && ((((xk).component [0]) != 0.) || (((xk).component [1]) != 0.)))
 {
  ...
  { ; (X [Li [j]]).component [0] -= (xk).component [0] * (Lval [j]).component [0] - (xk).component [1] * (Lval [j]).component [1] ; (X [Li [j]]).component [1] -= (xk).component [1] * (Lval [j]).component [0] + (xk).component [0] * (Lval [j]).component [1] ; } ;
     }
 }
    }
